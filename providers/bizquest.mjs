...
export default {
  ...
  buildSearchUrl(terms) {
    ...
    if (st && city) return `https://www.bizquest.com/businesses-for-sale-in-${city}-${st}/`;
    if (st) return `https://www.bizquest.com/businesses-for-sale-in-${stateName.replace(/\s+/g, '-')}-${st}/`;
  ...
  parseSearchResults(html, url) {
    ...
    for (const card of cards) {
      const hrefMatch = card.match(/href="([^"]*\/business-for-sale\/[^"]+)"/i);
  ...
  parseListing(content, url) {
    ...
    if (typeof content === 'string' && (content.includes('search-result-item') || content.includes('listing-card'))) {
    ...
    const priceMatch = text.match(/Asking Price:?\s*<\/span>\s*<span[^>]*class="[^"]*(?:value|price)[^"]*"[^>]*>([^<]*)<\/span>/i) ||
                       text.match(/Asking Price:?\s*<\/b>\s*([^<]+)/i) ||
                       text.match(/<span[^>]*class="[^"]*(?:asking-price|price)[^"]*"[^>]*>([^<]*)<\/span>/i) ||
    ...
    const descMatch = text.match(/<div[^>]*class="[^"]*(?:business-)?description[^"]*"[^>]*>([\s\S]*?)<\/div>/i) ||
                      text.match(/<div[^>]*class="[^"]*listing-description[^"]*"[^>]*>([\s\S]*?)<\/div>/i) ||
    ...
    if (htmlTitleMatch && htmlTitleMatch[1].trim()) {
      ...
      title = title.replace(/\s+-\s+[A-Za-z\s,]+(?:\|\s*BizQuest)?$/i, '').trim();
    }
