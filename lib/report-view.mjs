...
function parseFooterYaml(inner) {
...
// Locate a bare (unfenced) `--- ... ---` YAML document at EOF. The footer's
// opening marker is the second-to-last `---` line when the closing marker sits
// at end-of-file — the YAML body itself never contains a bare `---` line.
function bareFooter(md) {
  ...
  for (let i = last + 1; i < lines.length; i++) {
    if (lines[i].trim() !== '') return null; // closing --- must end the file
  }
  let open = -1;
  for (let i = last - 1; i >= 0; i--) {
    if (/^\s*---\s*$/.test(lines[i])) { open = i; break; }
  }
  if (open < 0) return null;
  const result = parseFooterYaml(lines.slice(open + 1, last).join('\n'));
  return result && typeof result === 'object' ? result : null;
}

export function parseYamlFooter(md) {
  if (!md) return null;
  // Preferred: a fenced ```yaml block at EOF (per modes/_shared.md Section 4).
  const fenced = md.match(/```yaml\s*\n([\s\S]*?)\n```\s*$/);
  if (fenced) return parseFooterYaml(fenced[1]);
  // Fallback: bare YAML document markers (--- ... ---) at EOF. Tolerates a
  // generator that skipped the ```yaml fence, so the dashboard still parses.
  return bareFooter(md);
}
...
export function renderReportPage({ deal, reportMd, benchmarks, artifacts = [] }) {
  ...
  const hasEbitda = footer.ebitda != null && Number.isFinite(Number(footer.ebitda));
  const ebitdaEst = hasEbitda && isEstimated('ebitda', footer.ebitda_estimated);
  ...
  const metricCards = [
    ...
    hasEbitda
      ? metricCard('EBITDA', money(footer.ebitda), {
          estimated: ebitdaEst,
          hint: 'EBITDA was not disclosed by the seller — this is a provisional estimate. Verify in due diligence.',
        })
      : '',
